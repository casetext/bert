...
def write_examples(job_id):
  ...
  input_files = _split(input_file_list,FLAGS.num_processes)[job_id]
  output_files = _split(output_file_list,FLAGS.num_processes)[job_id]
  
  if len(input_files)>1: 
      tf.logging.info("Worker {} Reading from {} input files: file {} through {}".format(job_id,len(input_files),
...
if __name__ == "__main__":
  ...
  if FLAGS.num_processes == 1:
    ...
    for i in range(FLAGS.num_processes):
      job = multiprocessing.Process(target=write_examples, args=(i,))
      jobs.append(job)
      job.start()
    for job in jobs:
      job.join()
